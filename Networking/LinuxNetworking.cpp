...
void SendData(const int& sendingSocketFileDescriptor, const void* sendBuffer, unsigned long& sendBufferLength) {
    if (send(sendingSocketFileDescriptor, sendBuffer, sendBufferLength, 0) == -1) throw std::runtime_error("Failed to send data.");
}

void ReceiveData(const int& receivingSocketFileDescriptor, void* receiveBuffer, unsigned long& receiveBufferLength) {
    if (recv(receivingSocketFileDescriptor, receiveBuffer, receiveBufferLength, 0) == -1) throw std::runtime_error("Failed to receive data.");
}
